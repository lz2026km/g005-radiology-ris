...
import { SHORTCUT_LIST, SHORTCUT_GROUPS, formatShortcut } from '../../config/shortcuts';
import type { UserConfig } from '../../config/userConfig';
import { DEFAULT_CONFIG } from '../../config/userConfig';
...
export function SettingsPanel({ config, updateConfig, resetConfig, updateField, trigger }: SettingsPanelProps) {
  const { t, i18n } = useTranslation();
  const [open, setOpen] = useState(false);
  const isZh = i18n.language?.startsWith('zh');

  const groupedShortcuts = SHORTCUT_LIST.reduce<Record<string, typeof SHORTCUT_LIST>>((acc, s) => {
    (acc[s.group] ??= []).push(s);
    return acc;
  }, {});
  ...
  return (
    <>
      ...
      >
        {trigger ?? <SettingOutlined style={{ fontSize: 18, color: 'var(--text-secondary)' }} />}
      </div>
                  ...
                  <div style={{ maxHeight: 360, overflowY: 'auto' }}>
                    {Object.entries(groupedShortcuts).map(([group, items]) => (
                      <div key={group} style={{ marginBottom: 20 }}>
                        <div style={{ fontSize: 12, fontWeight: 600, color: '#94a3b8', textTransform: 'uppercase', marginBottom: 8, letterSpacing: '0.05em' }}>
                          {isZh ? SHORTCUT_GROUPS[group]?.labelZh : SHORTCUT_GROUPS[group]?.labelEn}
                        </div>
                        {items.map((shortcut) => (
                          <div
                            key={shortcut.action}

...
import { Plus, Minus, RotateCw, Maximize2, Grid3x3, Sun, Layers, Heart, Activity, Wifi, WifiOff, Download } from 'lucide-react'
...
function Viewport({ image, preset, onPresetChange, offlineMode }: ViewportProps) {
  ...
  const handleCacheImage = () => {
    setCached(true)
    setTimeout(() => setCached(false), 1500)
  }
  ...
  return (
    <div style={{ position: 'relative', width: '100%', aspectRatio: '1', background: '#000', borderRadius: 8, overflow: 'hidden' }}
      ...
      <div style={{ position: 'absolute', right: 8, bottom: 8, display: 'flex', flexDirection: 'column', gap: 4 }}>
        ...
        {offlineMode && <button onClick={handleCacheImage} style={{ ...ctrlBtnStyle, background: cached ? '#16a34a' : 'rgba(255,255,255,0.2)' }}><Download size={14} /></button>}
      </div>
    </div>
  )
}
    ...
    <div style={{ maxWidth: 420, margin: '0 auto', background: '#0a0a0a', minHeight: '100vh', fontFamily: '-apple-system, sans-serif' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: `repeat(${gridCols}, 1fr)`, gap: 2, padding: 2 }}>
        {Array.from({ length: viewportCount }).map((_, idx) => (
          <Viewport key={idx} image={MOCK_IMAGES[currentImage + idx]} preset={preset} onPresetChange={setPreset} offlineMode={offlineMode} />
        ))}
      </div>

      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 2 }}>
            ...
            <span style={{ fontSize: 12, padding: '1px 6px', borderRadius: 4, background: style.color + '20', color: style.color }}>{alert.category}</span>
            {alert.blocking && <span style={{ fontSize: 12, padding: '1px 6px', borderRadius: 4, background: '#f8514920', color: '#f85149' }}>BLOCKING</span>}
          </div>
        ...
        <div style={{ display: 'flex', gap: 4, flexShrink: 0 }}>
          {onAcknowledge && alert.status === 'active' && (
            <button onClick={(e) => { e.stopPropagation(); onAcknowledge(alert.id) }}
              style={{ padding: '4px 10px', borderRadius: 4, border: '1px solid #30363d', background: '#21262d', color: '#8b949e', cursor: 'pointer', fontSize: 12 }}>确认</button>
          )}
          {onDismiss && (
        ...
        <div style={{ padding: '0 14px 12px 48px', borderTop: `1px solid ${style.border}20`, marginTop: 4 }}>
          {alert.recommendations && alert.recommendations.length > 0 && (
            <div style={{ marginBottom: 8 }}>
              <div style={{ fontSize: 12, color: '#6e7681', fontWeight: 600, marginBottom: 4 }}>推荐操作</div>
          ...
          {alert.evidence && alert.evidence.length > 0 && (
            <div>
              <div style={{ fontSize: 12, color: '#6e7681', fontWeight: 600, marginBottom: 4 }}>证据</div>

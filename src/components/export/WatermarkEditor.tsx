  ...
  return (
    <div style={{ background: '#fff', borderRadius: 10, border: '1px solid #e2e8f0', padding: 16 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        <div>
          <label style={{ fontSize: 11, color: '#475569', display: 'block', marginBottom: 2 }}>类型</label>
        ...
        {opts.type === 'text' && (
          <>
            <div>
              <label style={{ fontSize: 11, color: '#475569', display: 'block', marginBottom: 2 }}>文字</label>
              <input value={opts.text ?? ''} onChange={e => update({ text: e.target.value })} style={inputStyle} />
            </div>
            <div>
              <label style={{ fontSize: 11, color: '#475569', display: 'block', marginBottom: 2 }}>字号</label>
              <input type="number" value={opts.fontSize ?? 80} onChange={e => update({ fontSize: +e.target.value })} style={inputStyle} min={12} max={200} />
            </div>
            <div>
              <label style={{ fontSize: 11, color: '#475569', display: 'block', marginBottom: 2 }}>颜色</label>
        ...
        <div>
          <label style={{ fontSize: 11, color: '#475569', display: 'block', marginBottom: 2 }}>不透明度: {opts.opacity ?? 0.1}</label>
          <input type="range" min="0" max="1" step="0.05" value={opts.opacity ?? 0.1} onChange={e => update({ opacity: +e.target.value })} style={{ width: '100%' }} />
        </div>
        <div>
          <label style={{ fontSize: 11, color: '#475569', display: 'block', marginBottom: 2 }}><RotateCw size={11} /> 旋转: {opts.rotation ?? -30}°</label>
          <input type="range" min="-90" max="90" value={opts.rotation ?? -30} onChange={e => update({ rotation: +e.target.value })} style={{ width: '100%' }} />
        </div>
        <div>
          <label style={{ fontSize: 11, color: '#475569', display: 'block', marginBottom: 2 }}>位置</label>
        ...
        {opts.type === 'image' && (
          <div>
            <label style={{ fontSize: 11, color: '#475569', display: 'block', marginBottom: 2 }}>图片 URL</label>
      ...
      {previewHtml && (
        <div style={{ marginTop: 12 }}>
          <label style={{ fontSize: 11, color: '#475569', display: 'block', marginBottom: 4 }}><Type size={11} /> 预览</label>

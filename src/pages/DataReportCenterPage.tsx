...
import {
  ...
  // 新增图标
  GripVertical, Layers, Share2, GitBranch, Sliders, BarChart4, Table2,
  LayoutDashboard, Send, Repeat, FolderTree, PanelRight, Sigma,
  MousePointer, Copy, Save, Trash2, Move, Maximize2, Minimize2
} from 'lucide-react'
import {
  BarChart,
  Bar,
...
const ReportBuilder = () => {
  ...
  const addWidget = (widgetId) => {
    const newWidget = { id: `w${Date.now()}`, type: widgetId, label: widgetPaletteItems.find(w => w.id === widgetId)?.label || widgetId, config: { dataSource: 'examVolume', dimensions: [], filters: [] }, gridPos: { x: canvasWidgets.length % 3, y: Math.floor(canvasWidgets.length / 3), w: 1, h: 1 } }
  ...
  const renderWidgetPreview = (widget) => {
    ...
    return (
      <div style={baseStyle} onClick={() => openConfig(widget)}>
        ...
        <div style={{ fontSize: '24px', color: COLORS.primary, marginBottom: '8px' }}>{widget.type === 'bar' ? '📊' : widget.type === 'line' ? '📈' : widget.type === 'pie' ? '🥧' : widget.type === 'table' ? '📋' : widget.type === 'kpi' ? '📌' : '📝'}</div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      <div style={{ display: 'flex', gap: '16px' }}>
        <div style={{ ...styles.card, width: '220px', flexShrink: 0 }}>
          <div style={styles.cardHeader}><div style={styles.cardTitle}><Layers size={16} /> 组件面板</div></div>
        ...
        <div style={{ flex: 1 }}>
          <div style={styles.card}>
            <div style={styles.cardHeader}>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                <button style={{ ...styles.btn, ...styles.btnOutline }} onClick={() => setShowSaveDialog(true)}><Save size={14} /> 保存布局</button>
                <button style={{ ...styles.btn, ...styles.btnOutline }} onClick={() => setCanvasWidgets([])}><Trash2 size={14} /> 清空</button>
              </div>
            </div>
            <div style={{ padding: '16px', minHeight: '300px' }}>
              {canvasWidgets.length === 0 ? (
                <div style={styles.emptyState}><p style={{ margin: 0 }}>从左侧组件面板拖拽组件到此处开始构建报表</p></div>
              ) : (
          ...
          <div style={{ ...styles.card, marginTop: '12px' }}>
            <div style={styles.cardHeader}><div style={styles.cardTitle}><FolderTree size={16} /> 已保存布局</div></div>
            <div style={{ padding: '12px' }}>
              {savedLayouts.map(layout => (
                <div key={layout.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 12px', borderBottom: '1px solid #f1f5f9' }}>
                  <div><div style={{ fontWeight: 500, fontSize: '13px' }}>{layout.name}</div><div style={{ fontSize: '11px', color: COLORS.textMuted }}>{layout.widgets} 个组件 · {layout.lastModified} · {layout.createdBy}</div></div>
                  <div style={{ display: 'flex', gap: '6px' }}>
                    <button style={{ ...styles.btn, padding: '4px 10px', fontSize: '12px', ...styles.btnOutline }} onClick={() => loadLayout(layout)}><Eye size={12} /> 加载</button>
                  </div>
                </div>
              ))}
            </div>
      ...
      {showConfig && selectedConfig && (
        <div style={styles.modalOverlay} onClick={() => setShowConfig(false)}>
          <div style={styles.modal} onClick={e => e.stopPropagation()}>
            <div style={styles.modalHeader}><div style={{ fontSize: '16px', fontWeight: 600 }}>配置组件 - {selectedConfig.label}</div><button style={{ background: 'none', border: 'none', cursor: 'pointer' }} onClick={() => setShowConfig(false)}><X size={18} /></button></div>
            <div style={styles.modalBody}>
              <div style={{ marginBottom: '16px' }}>
                <div style={{ fontSize: '13px', fontWeight: 600, marginBottom: '6px' }}>数据来源</div>
              ...
              <div style={{ marginBottom: '16px' }}>
                <div style={{ fontSize: '13px', fontWeight: 600, marginBottom: '6px' }}>维度</div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                  {['月份', '设备类型', '科室', '医生'].map(d => <label key={d} style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '13px' }}><input type="checkbox" /> {d}</label>)}
                </div>
              </div>
              <div style={{ marginBottom: '16px' }}>
                <div style={{ fontSize: '13px', fontWeight: 600, marginBottom: '6px' }}>筛选条件</div>
                <div style={{ display: 'flex', gap: '8px' }}>
                  <select style={styles.select}><option value="">选择字段</option><option>设备类型</option><option>科室</option></select>
                  <select style={styles.select}><option value="=">=</option><option>&gt;</option><option>&lt;</option></select>
                  <input style={styles.input} placeholder="值" />
                  <button style={{ ...styles.btn, ...styles.btnPrimary, padding: '6px 12px', fontSize: '12px' }}>添加</button>
                </div>
              </div>
              <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                <button style={{ ...styles.btn, ...styles.btnPrimary }} onClick={() => setShowConfig(false)}>确认</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {showSaveDialog && (
        <div style={styles.modalOverlay} onClick={() => setShowSaveDialog(false)}>
          <div style={styles.modal} onClick={e => e.stopPropagation()}>
            <div style={styles.modalHeader}><div style={{ fontSize: '16px', fontWeight: 600 }}>保存报表布局</div><button style={{ background: 'none', border: 'none', cursor: 'pointer' }} onClick={() => setShowSaveDialog(false)}><X size={18} /></button></div>
            <div style={styles.modalBody}>
              <div style={{ marginBottom: '16px' }}>
                <div style={{ fontSize: '13px', fontWeight: 600, marginBottom: '6px' }}>布局名称</div>
                <input style={{ ...styles.input, width: '100%' }} placeholder="输入布局名称" value={layoutName} onChange={e => setLayoutName(e.target.value)} />
              </div>
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}>
                <button style={{ ...styles.btn, ...styles.btnOutline }} onClick={() => setShowSaveDialog(false)}>取消</button>
...
const ScheduledDistribution = () => {
  ...
  const addSchedule = () => {
    ...
    setShowAdd(false); setNewSchedule({ name: '', frequency: '每月', format: 'PDF', recipients: '' })
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      <div style={styles.card}>
        <div style={styles.cardHeader}>
          <div style={styles.cardTitle}><Send size={16} /> 调度任务</div>
          <button style={{ ...styles.btn, ...styles.btnPrimary }} onClick={() => setShowAdd(true)}><Plus size={14} /> 新建调度</button>
        </div>
        <table style={styles.table}>
          <thead><tr><th style={styles.th}>任务名称</th><th style={styles.th}>频率</th><th style={styles.th}>格式</th><th style={styles.th}>收件人</th><th style={styles.th}>上次发送</th><th style={styles.th}>下次发送</th><th style={styles.th}>状态</th><th style={styles.th}>操作</th></tr></thead>
      ...
      <div style={styles.card}>
        <div style={styles.cardHeader}><div style={styles.cardTitle}><Clock size={16} /> 分发日志</div></div>
        <table style={styles.table}>
          <thead><tr><th style={styles.th}>调度任务</th><th style={styles.th}>发送时间</th><th style={styles.th}>格式</th><th style={styles.th}>收件人</th><th style={styles.th}>状态</th></tr></thead>
          <tbody>{distLog.map(d => (
            <tr key={d.id}>
      ...
      {showAdd && (
        <div style={styles.modalOverlay} onClick={() => setShowAdd(false)}>
          <div style={styles.modal} onClick={e => e.stopPropagation()}>
            <div style={styles.modalHeader}><div style={{ fontSize: '16px', fontWeight: 600 }}>新建调度任务</div><button style={{ background: 'none', border: 'none', cursor: 'pointer' }} onClick={() => setShowAdd(false)}><X size={18} /></button></div>
            <div style={styles.modalBody}>
              <div style={{ marginBottom: '12px' }}><label style={{ display: 'block', fontSize: '13px', fontWeight: 600, marginBottom: '4px' }}>任务名称</label><input style={{ ...styles.input, width: '100%' }} value={newSchedule.name} onChange={e => setNewSchedule({ ...newSchedule, name: e.target.value })} /></div>
              <div style={{ marginBottom: '12px' }}><label style={{ display: 'block', fontSize: '13px', fontWeight: 600, marginBottom: '4px' }}>频率</label><select style={styles.select} value={newSchedule.frequency} onChange={e => setNewSchedule({ ...newSchedule, frequency: e.target.value })}><option>每日</option><option>每周</option><option>每月</option><option>每季度</option></select></div>
              <div style={{ marginBottom: '12px' }}><label style={{ display: 'block', fontSize: '13px', fontWeight: 600, marginBottom: '4px' }}>格式</label><select style={styles.select} value={newSchedule.format} onChange={e => setNewSchedule({ ...newSchedule, format: e.target.value })}><option>PDF</option><option>Excel</option><option>CSV</option><option>HTML</option></select></div>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}>
                <button style={{ ...styles.btn, ...styles.btnOutline }} onClick={() => setShowAdd(false)}>取消</button>
                <button style={{ ...styles.btn, ...styles.btnPrimary }} onClick={addSchedule}><Plus size={14} /> 创建</button>
              </div>
            </div>
          </div>
        </div>
...
const DrillDownNavigation = () => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      {drillLevel === 'summary' && (
        <div style={styles.card}>
          <div style={styles.cardHeader}><div style={styles.cardTitle}><BarChart3 size={16} /> 概要统计 - 点击分类下钻</div></div>
          <table style={styles.table}>
            <thead><tr><th style={styles.th}>分类</th><th style={styles.th}>检查量</th><th style={styles.th}>收入(元)</th><th style={styles.th}>成本(元)</th><th style={styles.th}>平均用时(min)</th><th style={styles.th}>操作</th></tr></thead>
            <tbody>{summaryData.map(item => (
              <tr key={item.category} style={{ cursor: 'pointer' }} onClick={() => drillDetail(item.category)}>
                ...
                <td style={styles.td}><button style={{ ...styles.btn, padding: '4px 8px', fontSize: '11px', ...styles.btnPrimary }} onClick={(e) => { e.stopPropagation(); drillDetail(item.category) }}>查看详情 <ChevronRight size={10} /></button></td>
              </tr>
            ))}</tbody>
          </table>
        </div>
      ...
      {drillLevel === 'detail' && (
        <div style={styles.card}>
          ...
          <table style={styles.table}>
            <thead><tr><th style={styles.th}>患者</th><th style={styles.th}>医生</th><th style={styles.th}>日期</th><th style={styles.th}>收入(元)</th><th style={styles.th}>成本(元)</th><th style={styles.th}>操作</th></tr></thead>
            <tbody>{detailRecords.map(r => (
              <tr key={r.id} style={{ cursor: 'pointer' }} onClick={() => drillToRecord(r)}>
                ...
                <td style={styles.td}><button style={{ ...styles.btn, padding: '4px 8px', fontSize: '11px', ...styles.btnOutline }} onClick={(e) => { e.stopPropagation(); drillToRecord(r) }}>查看 <ChevronRight size={10} /></button></td>
              </tr>
            ))}</tbody>
          </table>
        </div>
      ...
      {drillLevel === 'record' && (
        <div style={styles.card}>
          <div style={styles.cardHeader}><div style={styles.cardTitle}><User size={16} /> 患者详情</div></div>
          <div style={{ padding: '18px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
            <div><div style={{ fontSize: '12px', color: COLORS.textMuted }}>患者姓名</div><div style={{ fontWeight: 500 }}>{breadcrumb[2]}</div></div>
            <div><div style={{ fontSize: '12px', color: COLORS.textMuted }}>检查分类</div><div style={{ fontWeight: 500 }}>{selectedSegment}</div></div>
            <div><div style={{ fontSize: '12px', color: COLORS.textMuted }}>检查日期</div><div>2026-05-15</div></div>
            <div><div style={{ fontSize: '12px', color: COLORS.textMuted }}>报告医生</div><div>李明</div></div>
...
const OLAPFiltering = () => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      <div style={styles.card}>
        <div style={styles.cardHeader}><div style={styles.cardTitle}><Sliders size={16} /> 多维筛选</div></div>
        <div style={{ padding: '16px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px', marginBottom: '16px' }}>
            <div><div style={{ fontSize: '12px', color: COLORS.textMuted, marginBottom: '4px' }}>时间维度</div><select style={{ ...styles.select, width: '100%' }} defaultValue="月"><option>日</option><option>周</option><option>月</option><option>年</option></select></div>
            <div><div style={{ fontSize: '12px', color: COLORS.textMuted, marginBottom: '4px' }}>科室</div><select style={{ ...styles.select, width: '100%' }} defaultValue="全部"><option>全部</option><option>放射科</option><option>CT室</option><option>MRI室</option></select></div>
            <div><div style={{ fontSize: '12px', color: COLORS.textMuted, marginBottom: '4px' }}>设备类型</div><select style={{ ...styles.select, width: '100%' }} defaultValue="全部"><option>全部</option><option>CT</option><option>MR</option><option>DR</option></select></div>
            <div><div style={{ fontSize: '12px', color: COLORS.textMuted, marginBottom: '4px' }}>医生</div><select style={{ ...styles.select, width: '100%' }} defaultValue="全部"><option>全部</option><option>李明</option><option>王芳</option><option>赵强</option></select></div>
          </div>
          <div style={{ borderTop: '1px solid #e5e7eb', paddingTop: '12px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
              <div style={{ fontSize: '13px', fontWeight: 600 }}>筛选链</div>
              <button style={{ ...styles.btn, padding: '4px 10px', fontSize: '12px', ...styles.btnOutline }} onClick={() => setShowFilterBuilder(true)}><Plus size={12} /> 添加条件</button>
            </div>
            {filterChain.length === 0 ? (
              <div style={{ fontSize: '13px', color: COLORS.textMuted, textAlign: 'center', padding: '12px' }}>尚未添加筛选条件</div>
            ) : (
      ...
      <div style={styles.grid2}>
        <div style={styles.card}>
          <div style={styles.cardHeader}><div style={styles.cardTitle}><BarChart3 size={16} /> 维度分析</div></div>
        ...
        <div style={styles.card}>
          <div style={styles.cardHeader}><div style={styles.cardTitle}><PieChartIcon size={16} /> 分布</div></div>
          <div style={{ padding: '16px', height: '280px' }}>
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
      ...
      {showFilterBuilder && (
        <div style={styles.modalOverlay} onClick={() => setShowFilterBuilder(false)}>
          <div style={styles.modal} onClick={e => e.stopPropagation()}>
            <div style={styles.modalHeader}><div style={{ fontSize: '16px', fontWeight: 600 }}>添加筛选条件</div><button style={{ background: 'none', border: 'none', cursor: 'pointer' }} onClick={() => setShowFilterBuilder(false)}><X size={18} /></button></div>
            <div style={styles.modalBody}>
              <div style={{ marginBottom: '12px' }}><label style={{ display: 'block', fontSize: '13px', fontWeight: 600, marginBottom: '4px' }}>维度</label><select style={styles.select}><option>时间</option><option>科室</option><option>设备类型</option><option>医生</option></select></div>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}>
                <button style={{ ...styles.btn, ...styles.btnOutline }} onClick={() => setShowFilterBuilder(false)}>取消</button>
                <button style={{ ...styles.btn, ...styles.btnPrimary }} onClick={addFilter}>添加</button>
              </div>
            </div>
          </div>
        </div>
...
const BenchmarkComparison = () => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
        <button style={{ ...styles.btn, ...styles.btnPrimary }} onClick={() => setShowPeerInput(true)}><Plus size={14} /> 录入同行数据</button>
      </div>
      <div style={styles.card}>
        <div style={styles.cardHeader}><div style={styles.cardTitle}><BarChart4 size={16} /> 指标对比</div></div>
        <table style={styles.table}>
          <thead><tr><th style={styles.th}>指标</th><th style={styles.th}>本院</th><th style={styles.th}>标杆A</th><th style={styles.th}>标杆B</th><th style={styles.th}>标杆C</th><th style={styles.th}>百分位排名</th><th style={styles.th}>差距分析</th></tr></thead>
              ...
              <td style={styles.td}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <div style={{ ...styles.progressBar, width: '80px' }}>
                    <div style={{ ...styles.progressFill, width: `${item.percentile}%`, backgroundColor: item.percentile >= 70 ? COLORS.success : item.percentile >= 50 ? COLORS.warning : COLORS.danger }} />
                  </div>
                  <span style={{ fontSize: '12px', fontWeight: 600 }}>{item.percentile}%</span>
                </div>
              </td>
              <td style={styles.td}>
                <span style={{ color: item.gap.startsWith('+') ? COLORS.success : item.gap.startsWith('-') ? COLORS.danger : COLORS.textMuted, fontWeight: 500 }}>
                  {item.gap.startsWith('+') ? '↑' : item.gap.startsWith('-') ? '↓' : '→'} {item.gap}
                </span>
              </td>
            </tr>
          ))}</tbody>
      ...
      <div style={styles.grid2}>
        ...
        <div style={styles.card}>
          <div style={styles.cardHeader}><div style={styles.cardTitle}><Target size={16} /> 差距分析</div></div>
          <div style={{ padding: '16px' }}>
            {benchData.filter(d => d.gap.startsWith('-')).length === 0 ? (
              <div style={{ textAlign: 'center', padding: '40px', color: COLORS.success }}>
                ...
                <div style={{ fontWeight: 600 }}>所有指标均达到或超过标杆水平</div>
              </div>
            ) : (
              <div>
                <div style={{ fontSize: '13px', fontWeight: 600, marginBottom: '12px', color: COLORS.danger }}>需改进指标</div>
      ...
      {showPeerInput && (
        <div style={styles.modalOverlay} onClick={() => setShowPeerInput(false)}>
          <div style={styles.modal} onClick={e => e.stopPropagation()}>
            <div style={styles.modalHeader}><div style={{ fontSize: '16px', fontWeight: 600 }}>录入标杆数据</div><button style={{ background: 'none', border: 'none', cursor: 'pointer' }} onClick={() => setShowPeerInput(false)}><X size={18} /></button></div>
            <div style={styles.modalBody}>
              <div style={{ marginBottom: '12px' }}><label style={{ display: 'block', fontSize: '13px', fontWeight: 600, marginBottom: '4px' }}>机构名称</label><input style={{ ...styles.input, width: '100%' }} placeholder="如: 标杆医院A" /></div>
              <div style={{ marginBottom: '12px' }}><label style={{ display: 'block', fontSize: '13px', fontWeight: 600, marginBottom: '4px' }}>数据文件</label><input type="file" style={{ ...styles.input, width: '100%' }} /></div>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}>
                <button style={{ ...styles.btn, ...styles.btnOutline }} onClick={() => setShowPeerInput(false)}>取消</button>
      ...
      case 'examVolume':
        return (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div style={styles.card}>
              <div style={styles.cardHeader}>
                <div style={styles.cardTitle}><BarChart3 size={18} /> 月度检查量趋势</div>
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button style={{ ...styles.btn, ...styles.btnOutline }} onClick={() => handleExport('examVolume')}><Download size={14} /> 导出Excel</button>
                  <button style={{ ...styles.btn, ...styles.btnPrimary }} onClick={() => { setUploadDataType('examVolume'); setShowUploadModal(true); }}><Upload size={14} /> 上报数据</button>
                </div>
              </div>
            ...
            <div style={styles.grid2}>
              <div style={styles.card}>
                <div style={styles.cardHeader}><div style={styles.cardTitle}><PieChartIcon size={18} /> 设备类型分布</div></div>
                <div style={{ padding: '18px', height: '280px' }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      ...
                      ]} cx="50%" cy="50%" outerRadius={90} dataKey="value" label={({ name, percent }) => `${name} ${(percent * 100).toFixed(1)}%`}>
              ...
              <div style={styles.card}>
                <div style={styles.cardHeader}>
                  <div style={styles.cardTitle}><Activity size={18} /> 待上报检查列表</div>
                  <span style={{ ...styles.statusBadge, backgroundColor: COLORS.warningLight, color: COLORS.warning }}>{pendingReports.length} 条待处理</span>
                </div>
      ...
      case 'deviceUsage':
        return (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div style={styles.card}>
              <div style={styles.cardHeader}>
                <div style={styles.cardTitle}><Gauge size={18} /> 各设备使用率</div>
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button style={{ ...styles.btn, ...styles.btnOutline }} onClick={() => handleExport('deviceUsage')}><Download size={14} /> 导出报表</button>
                  <button style={{ ...styles.btn, ...styles.btnPrimary }} onClick={() => { setUploadDataType('deviceUsage'); setShowUploadModal(true); }}><Upload size={14} /> 上报数据</button>
                </div>
              </div>
            ...
            <div style={styles.card}>
              ...
              <table style={styles.table}>
                <thead><tr><th style={styles.th}>设备名称</th><th style={styles.th}>使用率</th><th style={styles.th}>平均报告时间</th><th style={styles.th}>状态</th><th style={styles.th}>操作</th></tr></thead>
                <tbody>{deviceUsageData.map((device, idx) => (
                  <tr key={idx}>
                    ...
                    <td style={styles.td}><button style={{ ...styles.btn, ...styles.btnOutline, padding: '4px 10px' }} onClick={() => setSelectedDevice(device)}><Eye size={12} /> 查看</button></td>
                  </tr>
                ))}</tbody>
              </table>
            </div>
      ...
      case 'qualityScore':
        return (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div style={styles.card}>
              <div style={styles.cardHeader}>
                <div style={styles.cardTitle}><Award size={18} /> 报告质量评分</div>
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button style={{ ...styles.btn, ...styles.btnOutline }} onClick={() => handleExport('qualityScore')}><Download size={14} /> 导出报表</button>
                  <button style={{ ...styles.btn, ...styles.btnPrimary }} onClick={() => { setUploadDataType('qualityScore'); setShowUploadModal(true); }}><Upload size={14} /> 上报数据</button>
                </div>
              </div>
            ...
            <div style={styles.grid2}>
              <div style={styles.card}>
                <div style={styles.cardHeader}><div style={styles.cardTitle}><Target size={18} /> 评分等级分布</div></div>
                <table style={styles.table}>
                  <thead><tr><th style={styles.th}>设备类型</th><th style={styles.th}>优秀率</th><th style={styles.th}>良好率</th><th style={styles.th}>合格率</th><th style={styles.th}>不合格</th></tr></thead>
                  <tbody>{qualityScoreData.map((item, idx) => (
                    <tr key={idx}><td style={styles.td}>{item.name}</td><td style={styles.td}><span style={{ color: COLORS.success, fontWeight: 500 }}>{item.excellent}%</span></td><td style={styles.td}>{item.good}%</td><td style={styles.td}>{item.pass}%</td><td style={styles.td}><span style={{ color: item.fail > 0 ? COLORS.danger : COLORS.textMuted }}>{item.fail}%</span></td></tr>
                  ))}</tbody>
                </table>
              </div>
              <div style={styles.card}>
                <div style={styles.cardHeader}><div style={styles.cardTitle}><CheckCircle size={18} /> 质量控制要点</div></div>
                <div style={{ padding: '18px' }}>
                  <div style={{ marginBottom: '16px' }}><div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}><span style={{ fontSize: '13px', color: COLORS.textMuted }}>报告完整率</span><span style={{ fontWeight: 600 }}>98.5%</span></div><div style={styles.progressBar}><div style={{ ...styles.progressFill, width: '98.5%', backgroundColor: COLORS.success }} /></div></div>
                  <div style={{ marginBottom: '16px' }}><div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}><span style={{ fontSize: '13px', color: COLORS.textMuted }}>诊断符合率</span><span style={{ fontWeight: 600 }}>96.2%</span></div><div style={styles.progressBar}><div style={{ ...styles.progressFill, width: '96.2%', backgroundColor: COLORS.primaryLight }} /></div></div>
                  <div style={{ marginBottom: '16px' }}><div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}><span style={{ fontSize: '13px', color: COLORS.textMuted }}>时效达标率</span><span style={{ fontWeight: 600 }}>94.8%</span></div><div style={styles.progressBar}><div style={{ ...styles.progressFill, width: '94.8%', backgroundColor: COLORS.mri }} /></div></div>
                  <div><div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}><span style={{ fontSize: '13px', color: COLORS.textMuted }}>危急值通报率</span><span style={{ fontWeight: 600 }}>100%</span></div><div style={styles.progressBar}><div style={{ ...styles.progressFill, width: '100%', backgroundColor: COLORS.success }} /></div></div>
                </div>
              </div>
            </div>
          </div>
      ...
      case 'doseStats':
        return (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div style={styles.card}>
              <div style={styles.cardHeader}>
                <div style={styles.cardTitle}><Zap size={18} /> 辐射剂量趋势</div>
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button style={{ ...styles.btn, ...styles.btnOutline }} onClick={() => handleExport('doseStats')}><Download size={14} /> 导出报表</button>
                  <button style={{ ...styles.btn, ...styles.btnPrimary }} onClick={() => { setUploadDataType('doseStats'); setShowUploadModal(true); }}><Upload size={14} /> 上报数据</button>
                </div>
              </div>
            ...
            <div style={styles.grid3}>
              <div style={styles.card}>
                ...
                <div style={{ padding: '18px' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                    ...
                    <div style={{ backgroundColor: '#f0f9ff', padding: '10px', borderRadius: '6px', textAlign: 'center' }}><div style={{ fontSize: '18px', fontWeight: 600, color: COLORS.ct }}>2,890</div><div style={{ fontSize: '11px', color: COLORS.textMuted }}>检查例数</div></div>
                  </div>
                </div>
              </div>
              <div style={styles.card}>
                ...
                <div style={{ padding: '18px' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                    <div style={{ backgroundColor: '#ecfdf5', padding: '10px', borderRadius: '6px', textAlign: 'center' }}><div style={{ fontSize: '18px', fontWeight: 600, color: COLORS.dr }}>4,800</div><div style={{ fontSize: '11px', color: COLORS.textMuted }}>检查例数</div></div>
              ...
              <div style={styles.card}>
                ...
                <div style={{ padding: '18px' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                    <div style={{ backgroundColor: '#fffbeb', padding: '10px', borderRadius: '6px', textAlign: 'center' }}><div style={{ fontSize: '18px', fontWeight: 600, color: COLORS.mg }}>460</div><div style={{ fontSize: '11px', color: COLORS.textMuted }}>检查例数</div></div>
            ...
            <div style={styles.card}>
              <div style={styles.cardHeader}><div style={styles.cardTitle}><ShieldCheck size={18} /> 剂量合规检查</div></div>
              <table style={styles.table}>
                <thead><tr><th style={styles.th}>检查类型</th><th style={styles.th}>检查例数</th><th style={styles.th}>平均DLP</th><th style={styles.th}>参考水平</th><th style={styles.th}>超标例数</th><th style={styles.th}>合规率</th></tr></thead>
                <tbody>
      ...
      case 'consultation':
        return (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div style={styles.grid2}>
              <div style={styles.card}>
                <div style={styles.cardHeader}><div style={styles.cardTitle}><Video size={18} /> 会诊类型分布</div><button style={{ ...styles.btn, ...styles.btnOutline }} onClick={() => handleExport('consultation')}><Download size={14} /> 导出报表</button></div>
                <div style={{ padding: '18px' }}><ConsultationPieChart data={consultationData} /></div>
              </div>
              <div style={styles.card}>
                <div style={styles.cardHeader}><div style={styles.cardTitle}><Users size={18} /> 会诊完成情况</div></div>
                <table style={styles.table}>
                  <thead><tr><th style={styles.th}>会诊类型</th><th style={styles.th}>总数</th><th style={styles.th}>已完成</th><th style={styles.th}>进行中</th><th style={styles.th}>平均耗时</th></tr></thead>
            ...
            <div style={styles.card}>
              <div style={styles.cardHeader}><div style={styles.cardTitle}><MessageSquare size={18} /> 会诊记录列表</div><button style={{ ...styles.btn, ...styles.btnPrimary }} onClick={() => setShowNewConsultationModal(true)}><Plus size={14} /> 新建会诊</button></div>
              <table style={styles.table}>
                <thead><tr><th style={styles.th}>会诊编号</th><th style={styles.th}>患者姓名</th><th style={styles.th}>会诊类型</th><th style={styles.th}>申请医生</th><th style={styles.th}>会诊医生</th><th style={styles.th}>申请时间</th><th style={styles.th}>状态</th></tr></thead>
                <tbody>
    ...
    <div style={styles.pageContainer}>
      <div style={styles.header}>
        <div>
          ...
          <div style={styles.headerSubtitle}>数据导出上报 / 检查量统计 / 设备使用率 / 报告质量 / 辐射剂量 / 会诊统计 / 自定义报表 / 调度分发 / 下钻导航 / OLAP筛选 / 标杆对比</div>
        </div>
      ...
      <div style={styles.statsContainer}>
        <StatCard icon={BarChart3} label="总检查量" value="17,816" unit="例" change="↑ 12.5% 较上月" changeType="up" color={COLORS.primary} />
        <StatCard icon={Gauge} label="设备使用率" value={avgDeviceUsage} unit="%" change="↑ 3.2% 较上月" changeType="up" color={COLORS.secondary} />
        <StatCard icon={Award} label="平均质量评分" value={avgQualityScore} unit="分" change="↑ 0.8 较上月" changeType="up" color={COLORS.success} />
        <StatCard icon={Zap} label="本月总剂量" value="46,800" unit="mGy·cm" change="↓ 2.1% 较上月" changeType="down" color={COLORS.warning} />
        <StatCard icon={Video} label="会诊总数" value={totalConsultations} unit="例" change="↑ 8.3% 较上月" changeType="up" color={COLORS.mri} />
      </div>
      ...
      <div style={styles.mainContent}>
        <div style={styles.card}>
          <div style={styles.filterBar}>
            <div style={styles.filterGroup}><span style={styles.filterLabel}>日期:</span><select style={styles.select} value={dateRange} onChange={(e) => setDateRange(e.target.value)}><option value="2026-05">2026年5月</option><option value="2026-04">2026年4月</option><option value="2026-03">2026年3月</option></select></div>
            <div style={styles.filterGroup}><span style={styles.filterLabel}>设备类型:</span><select style={styles.select} value={modality} onChange={(e) => setModality(e.target.value)}><option value="全部">全部</option><option value="CT">CT</option><option value="MR">MR</option><option value="DR">DR</option><option value="MG">MG</option><option value="DSA">DSA</option></select></div>
            <div style={{ flex: 1 }} />
            <button style={{ ...styles.btn, ...styles.btnOutline }} onClick={() => setShowAdvancedFilter(!showAdvancedFilter)}><Filter size={14} /> 高级筛选</button>
          </div>
          {renderTabContent()}
        </div>
      </div>
      ...
      {showExportModal && (
        <div style={styles.modalOverlay} onClick={() => setShowExportModal(false)}>
          <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
            <div style={styles.modalHeader}><div style={{ fontSize: '16px', fontWeight: 600 }}>确认导出</div><button style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '4px' }} onClick={() => setShowExportModal(false)}><X size={18} /></button></div>
            <div style={styles.modalBody}>
              <p style={{ marginBottom: '16px' }}>确定要导出 <strong>{exportType === 'examVolume' ? '检查量统计' : exportType === 'deviceUsage' ? '设备使用率' : exportType === 'qualityScore' ? '报告质量评分' : exportType === 'doseStats' ? '辐射剂量统计' : '会诊统计'}</strong> 数据吗？</p>
      ...
      {showUploadModal && (
        <div style={styles.modalOverlay} onClick={() => setShowUploadModal(false)}>
          <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
            <div style={styles.modalHeader}><div style={{ fontSize: '16px', fontWeight: 600 }}>确认上报</div><button style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '4px' }} onClick={() => setShowUploadModal(false)}><X size={18} /></button></div>
            <div style={styles.modalBody}>
              <p style={{ marginBottom: '16px' }}>确定要上报 <strong>{uploadDataType === 'examVolume' ? '检查量统计' : uploadDataType === 'deviceUsage' ? '设备使用率' : uploadDataType === 'qualityScore' ? '报告质量评分' : uploadDataType === 'doseStats' ? '辐射剂量统计' : '会诊统计'}</strong> 数据吗？</p>
              <div style={{ backgroundColor: '#f8fafc', padding: '12px', borderRadius: '6px', marginBottom: '16px' }}>
                <div style={{ fontSize: '13px', color: COLORS.textMuted, marginBottom: '8px' }}>上报说明</div>
                <div style={{ fontSize: '13px' }}>上报数据将加密传输至卫生健康委员会数据平台，请确认数据准确性。</div>
              </div>
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}>
      ...
      {selectedDevice && (
        <div style={styles.modalOverlay} onClick={() => setSelectedDevice(null)}>
          <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
            <div style={styles.modalHeader}><div style={{ fontSize: '16px', fontWeight: 600 }}>设备详情 - {selectedDevice.name}</div><button style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '4px' }} onClick={() => setSelectedDevice(null)}><X size={18} /></button></div>
            <div style={styles.modalBody}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
                <div><div style={{ fontSize: '12px', color: COLORS.textMuted, marginBottom: '4px' }}>设备名称</div><div style={{ fontWeight: 500 }}>{selectedDevice.name}</div></div>
                <div><div style={{ fontSize: '12px', color: COLORS.textMuted, marginBottom: '4px' }}>使用率</div><div style={{ fontWeight: 500, color: selectedDevice.usage >= 80 ? COLORS.success : selectedDevice.usage >= 60 ? COLORS.warning : COLORS.danger }}>{selectedDevice.usage}%</div></div>
                <div><div style={{ fontSize: '12px', color: COLORS.textMuted, marginBottom: '4px' }}>平均报告时间</div><div style={{ fontWeight: 500 }}>{selectedDevice.avgReport} min</div></div>
                <div><div style={{ fontSize: '12px', color: COLORS.textMuted, marginBottom: '4px' }}>状态</div><div style={{ fontWeight: 500, color: selectedDevice.status === '正常运行' ? COLORS.success : COLORS.warning }}>{selectedDevice.status}</div></div>
              </div>
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}><button style={{ ...styles.btn, ...styles.btnOutline }} onClick={() => setSelectedDevice(null)}>关闭</button></div>
            </div>
          </div>
        </div>
      )}
      ...
      {showNewConsultationModal && (
        <div style={styles.modalOverlay} onClick={() => setShowNewConsultationModal(false)}>
          <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
            <div style={styles.modalHeader}><div style={{ fontSize: '16px', fontWeight: 600 }}>新建会诊</div><button style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '4px' }} onClick={() => setShowNewConsultationModal(false)}><X size={18} /></button></div>
            <div style={styles.modalBody}>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}><button style={{ ...styles.btn, ...styles.btnOutline }} onClick={() => setShowNewConsultationModal(false)}>关闭</button></div>
            </div>
          </div>
        </div>
      )}
      ...
      {showAdvancedFilter && (
        <div style={{ ...styles.card, marginTop: '-8px' }}>
          <div style={{ padding: '16px 18px', display: 'flex', gap: '12px', flexWrap: 'wrap', alignItems: 'center' }}>
            <div style={styles.filterGroup}><span style={styles.filterLabel}>报告医生:</span><input style={styles.input} placeholder="请输入医生姓名" /></div>
            <div style={styles.filterGroup}><span style={styles.filterLabel}>患者姓名:</span><input style={styles.input} placeholder="请输入患者姓名" /></div>
            <div style={styles.filterGroup}><span style={styles.filterLabel}>检查类型:</span><select style={styles.select}><option value="">全部</option><option value="CT">CT</option><option value="MR">MR</option><option value="DR">DR</option><option value="MG">MG</option><option value="DSA">DSA</option></select></div>
            <button style={{ ...styles.btn, ...styles.btnPrimary }} onClick={() => setShowAdvancedFilter(false)}>应用筛选</button>
            <button style={{ ...styles.btn, ...styles.btnOutline }} onClick={() => setShowAdvancedFilter(false)}>重置</button>
          </div>
        </div>
      )}
    </div>

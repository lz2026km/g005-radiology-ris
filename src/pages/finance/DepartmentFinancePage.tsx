...
export default function DepartmentFinancePage() {
  ...
  const periodLabels: Record<Period, string> = { monthly: "月度", quarterly: "季度", yearly: "年度" }
  const handlePeriodChange = (p: Period) => {
    setPeriod(p)
    setExportMsg("已切换到 " + periodLabels[p] + " 视图 " + new Date().toLocaleTimeString("zh-CN"))
  }
  const handleExport = () => {
    setExportMsg("已导出 " + periodLabels[period] + " 财务报告 " + new Date().toLocaleTimeString("zh-CN"))
  }

  return (
    <div style={{ minHeight: "100vh", background: "#0d1117", color: "#f0f6fc", fontSize: 14, fontFamily: '"Segoe UI",sans-serif' }}>
      <div style={{ background: "linear-gradient(135deg,#1e40af,#1e3a8a)", padding: "16px 24px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button onClick={handleExport} style={{ padding: "6px 14px", borderRadius: 6, border: "none", cursor: "pointer", fontSize: 13, background: "rgba(255,255,255,0.15)", color: "#fff", display: "flex", alignItems: "center", gap: 6 }}>
              <Download size={14} />导出
            </button>
          </div>
        </div>
        {exportMsg && <div style={{ fontSize: 12, color: "#fff", opacity: 0.85, padding: "8px 0 0" }}>{exportMsg}</div>}
      </div>

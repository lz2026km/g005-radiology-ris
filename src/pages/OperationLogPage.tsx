// TODO v3.0.4: 此文件超过 2000 行（2804行），需要拆分为子组件
// v3.0.4 重构目标：
...
export default function OperationLogPage() {
  ...
  const showToast = (msg: string, type: 'success' | 'error' | 'info' = 'success') => { setToastMsg(msg); setTimeout(() => setToastMsg(null), 3000) }
  ...
  return (
    <div data-testid="operation-log-page" style={{ minHeight: '100vh', background: BG }}>
      ...
      <div style={{ background: WHITE, borderRadius: 10, padding: '4px', marginBottom: 16, display: 'flex', gap: 4, border: `1px solid #e2e8f0`, flexWrap: 'wrap' }}>
        {[
      ...
      {liveTab === 'complianceReports' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16, marginBottom: 16 }}>
          <div style={{ background: WHITE, borderRadius: 10, padding: 16, border: '1px solid #e2e8f0', display: 'flex', gap: 16, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <button onClick={() => { showToast('CSV已导出', 'success') }} style={{
            ...
            <button onClick={() => { showToast('PDF已导出', 'success') }} style={{
      ...
      {liveTab === 'blockchain' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16, marginBottom: 16 }}>
          ...
          <div style={{ background: WHITE, borderRadius: 12, border: '1px solid #e2e8f0', overflow: 'hidden' }}>
            ...
            {blockchainData.slice(0, 10).map((b, idx) => (
              <div key={b.id} style={{
                ...
                <div style={{ textAlign: 'center' }}>
                  {b.verified ? <CheckCircle size={12} color={SUCCESS} /> : <X size={12} color={DANGER} />}
                </div>
              </div>
            ))}
          </div>

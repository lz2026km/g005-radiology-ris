...
import {
  User, Scan, Monitor, Radio, Stethoscope, AlertTriangle,
  CheckSquare, Square, Eye, ArrowUp, ArrowDown,
} from 'lucide-react'
...
const PRIORITY_CONFIG: Record<string, { bg: string; color: string; label: string; order: number }> = {
...
export function ListView({ exams, selectedIds, onSelect, onRowClick, allSelected }: ListViewProps) {
                    ...
                    {(() => {
                      ...
                      return (
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                          ...
                          <span style={{ fontSize: 11, fontWeight: 600, color: sla.color }}>
                            {sla.elapsedMinutes}m
                          </span>
                          <span style={{ fontSize: 9, padding: '1px 4px', borderRadius: 3, background: autoPri.bg, color: autoPri.color }}>
                            {autoPri.score}
                          </span>
                        </div>
                      )

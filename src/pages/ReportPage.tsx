// HERMES-UNIQUE-UUID-A7B3C9D2-E5F6-4B8C-9D0E-F1A2B3C4D5E6
// @ts-nocheck
// BUILD-TEST-MARKER-2026-05-04-HERMES-TEST-12345
// ============================================================
...
function genMockReports(): RadiologyReport[] {
  ...
  const statuses: RadiologyReport["status"][] = [
    "已提交",
    "已审核",
    "已发布",
    "已修订",
  ...
  return initialRadiologyExams.map((exam, i) => ({
    ...
    criticalFinding: [true, false, false, true, false, true, false, false][
      i % 8
    ],
    ...
    auditorName:
      i % 2 === 0 ? DOCTORS[(i + 1) % DOCTORS.length].name : undefined,
    ...
    publishedBy:
      i % 3 === 0 ? DOCTORS[(i + 2) % DOCTORS.length].name : undefined,
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
          ...
          {MODALITIES.map((m) => (
            <button
              ...
              style={btnStyle(modalityFilter === m, ACCENT)}
            >
              {m}
            </button>
          ))}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
          ...
          {STATUSES.map((s) => (
            <button
              ...
              style={btnStyle(statusFilter === s, PURPLE)}
            >
              {s}
            </button>
          ))}
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1 }}>
        ...
        <div style={{ fontSize: 12, color: GRAY, fontWeight: 500 }}>
          {label}
        </div>
        {sub && (
...
function ListView({
  ...
  const SortIcon = ({ field }: { field: string }) =>
    sortField === field ? (
      <span style={{ marginLeft: 3, color: PRIMARY }}>
        {sortDir === "asc" ? "↑" : "↓"}
      </span>
    ) : (
        ...
        >
          ...
          <tbody>
            {paginated.map((r, idx) => {
              ...
              const hasPositive =
                r.diagnosis && r.diagnosis !== "结论：未见明显异常。";
              return (
                <>
                  <tr
                    key={r.id}
                          ...
                          {[
                            {
                              label: "起草",
                              time: r.createdTime,
                              name: r.reportDoctorName,
                              active: !!r.createdTime,
                            },
                            {
                              label: "审核",
                              time: r.approvedTime,
                              name: r.auditorName || r.auditorName,
                              active: !!r.approvedTime,
                            },
                            {
                              label: "签发",
                              time: r.signedTime,
      ...
      {totalPages > 1 && (
        ...
        >
          <span style={{ fontSize: 12, color: GRAY }}>
            显示 {(safePage - 1) * pageSize + 1}-
            {Math.min(safePage * pageSize, sorted.length)} 条,共 {sorted.length}{" "}
            条
          </span>
          <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
            ...
            {Array.from({ length: Math.min(7, totalPages) }, (_, i) => {
              let page = i + 1;
              if (totalPages > 7) {
                if (safePage > 4) page = safePage - 3 + i;
                if (safePage > totalPages - 3) page = totalPages - 6 + i;
              }
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 4,
                    border: "none",
                    background: safePage === page ? PRIMARY : "#fff",
                    color: safePage === page ? "#fff" : GRAY,
                    fontSize: 12,
                    fontWeight: safePage === page ? 700 : 400,
                    cursor: "pointer",
                    minWidth: 28,
                  }}
                >
            ...
            <button
              ...
              style={{
                ...
                border: "1px solid #e2e8f0",
                background: "#fff",
                color: safePage >= totalPages ? "#cbd5e1" : GRAY,
                fontSize: 12,
...
const KANBAN_COLUMNS = [
  ...
  },
  {
    key: "审核组",
    label: "👁️ 审核",
    subStatus: ["已提交", "初审中", "初审通过", "终审中", "已审核"],
    color: "#7c2d12",
    bg: "#fff7ed",
    border: "#fed7aa",
  },
...
function KanbanView({ reports, onView, onReview }: KanbanViewProps) {
  ...
  const columns = useMemo(() => {
    return KANBAN_COLUMNS.map((col) => ({
      ...col,
      items: reports.filter((r) =>
        (col.subStatus as readonly string[]).includes(r.status),
      ),
    }));
  }, [reports]);
        ...
        <div key={col.key} style={{ display: "flex", flexDirection: "column" }}>
          ...
          <div
            ...
            style={{
              ...
              background:
                dragOverCol === col.key ? `${col.color}08` : "#fafbfc",
          ...
          >
            {col.items.map((r) => (
              ...
              >
                ...
                >
                  {r.diagnosis
                    ? highlightAnomalies(r.diagnosis.slice(0, 50))
                    : "(无诊断)"}
...
function DetailModal({
  ...
  const [tab, setTab] = useState<"content" | "history" | "print" | "timeline">(
    "content",
  );
          ...
          {[
            {
              icon: <User size={13} />,
              label: "患者",
              value: `${report.patientName} (${report.gender}/${report.age}岁/${report.patientType})`,
            },
            {
              icon: <Stethoscope size={13} />,
              label: "检查",
              value: `${report.examItemName} (${report.modality})`,
            },
            {
            ...
            },
            {
              ...
              label: "设备",
              value: report.deviceName?.split("（")[0] || "-",
            },
          ].map((item) => (
            <div
              key={item.label}
              ...
              >
                ...
                >
                  ...
                  <StatusBadge
                    ...
                    showIcon={false}
                  />{" "}
                  状态。 系统支持 14 态状态机：待分配 → 已分配 → 书写中 → 已提交
                  → 初审中 → 初审通过 → 终审中 → 已审核 → 签发中 → 已签发 →
                  已发布 → 修订中 → 已修订 / 已撤回 / 已驳回 / 已归档。
                </div>
              </div>
            ...
            <div>
              ...
              {report.criticalFinding && (
                ...
                >
                  <Zap
                    size={16}
                    style={{ color: DANGER, flexShrink: 0, marginTop: 1 }}
                  />
                  <div>
                    <div
                      style={{
                    ...
                    <div style={{ fontSize: 12, color: "#7f1d1d" }}>
                      {report.criticalFindingDetails || report.diagnosis}
                    </div>
                  </div>
                </div>
              )}
              ...
              >
                ...
                >
                  ...
                  {report.impression &&
                    report.impression !== report.diagnosis && (
                      <>
                        <div
                          style={{
                            fontSize: 11,
              ...
              >
                ...
                >
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: "#475569" }}>
                      {report.qualityScore ? `${report.qualityScore}分` : "-"}
                    </div>
                  </div>
                  <div>
                    <div style={{ fontSize: 10, color: GRAY, marginBottom: 2 }}>
                  ...
                  >
                    <tbody>
                      {[
                        ...
                        ["检查类型", `${report.patientType}患者`],
                        ["检查日期", report.examDate],
                        ["检查项目", report.examItemName],
                        ["设备", report.deviceName?.split("（")[0] || "-"],
                ...
                <div style={{ marginBottom: 14 }}>
                  ...
                  <div
                    style={{
                      ...
                      fontWeight: 700,
                      padding: "8px 12px",
                      background: report.criticalFinding
                        ? "#fff5f5"
                        : "#f0fdf4",
                      borderRadius: 4,
                      border: `1px solid ${report.criticalFinding ? "#fca5a5" : "#bbf7d0"}`,
                    }}
                  >
        ...
        <div style={{ padding: 20 }}>
          ...
          >
            ...
            <div style={{ fontSize: 12, color: "#475569", lineHeight: 1.6 }}>
              <span style={{ fontWeight: 600 }}>{report.examItemName}</span> (
              {report.modality} · {report.bodyPart})
              <span style={{ color: GRAY, marginLeft: 8 }}>
                by {report.reportDoctorName}
              </span>
            </div>
            <div
              style={{
            ...
            {report.criticalFinding && (
              ...
              >
                <Zap size={11} /> 含危急值:{" "}
                {report.criticalFindingDetails?.slice(0, 60) ||
                  report.diagnosis}
              </div>
            )}
          </div>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            >
              审核结论 <span style={{ color: DANGER }}>*</span>
            </div>
            <div
              style={{
                display: "grid",
            ...
            >
              {[
                ...
                <button
                  key={opt.key}
                  onClick={() => setResult(opt.key as any)}
                  style={{
                    padding: "12px 16px",
                    borderRadius: 8,
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            >
              ...
              <input
                ...
                onFocus={(e) => {
                  if (!error) e.target.style.border = `1px solid ${ACCENT}`;
                }}
                onBlur={(e) => {
...
export default function ReportPage() {
  ...
  const [bulkActionModal, setBulkActionModal] = useState<{
    ...
    count: number;
    loading: boolean;
  }>({ show: false, action: "", count: 0, loading: false });
  ...
  const [exportModal, setExportModal] = useState<{
    ...
    message: string;
    complete: boolean;
  }>({ show: false, title: "", message: "", complete: false });
  ...
  const [reviewResultModal, setReviewResultModal] = useState<{
    ...
    result: string;
    suggestion: string;
  }>({ show: false, reportId: "", result: "", suggestion: "" });
  ...
  // 弹窗状态
  const [detailReport, setDetailReport] = useState<RadiologyReport | null>(
    null,
  );
  const [reviewReport, setReviewReport] = useState<RadiologyReport | null>(
    null,
  );
  ...
  const stats = useMemo(() => {
    ...
    const published = allReports.filter(
      (r) => r.publishedTime && r.createdTime,
    );
    let avgTurnaround = 0;
    ...
    return {
      ...
      positiveCount: allReports.filter(
        (r) => r.diagnosis && r.diagnosis !== "结论：未见明显异常。",
      ).length,
      avgTurnaround,
    };
  }, [allReports]);
  ...
  const filteredReports = useMemo(() => {
    return allReports.filter((r) => {
      ...
      // 设备
      if (modalityFilter !== "全部" && r.modality !== modalityFilter)
        return false;
      // 报告医生
      if (reportDoctorFilter && r.reportDoctorName !== reportDoctorFilter)
        return false;
      // 审核医生
  ...
  const handleExport = () => {
    setExportModal({
      show: true,
      title: "导出报表",
      message: `正在导出 ${filteredReports.length} 份报告...`,
      complete: false,
    });
    setTimeout(() => {
      setExportModal((m) => ({
        ...m,
        complete: true,
        message: `已导出 ${filteredReports.length} 份报告`,
      }));
  ...
  const handleReviewSubmit = async (
    ...
    setReviewResultModal({
      show: true,
      reportId,
      result: result === "approved" ? "已审核" : "已退回",
  ...
  return (
    ...
    >
      {accessDenied && (
        ...
        >
          🔒 资源级访问被拒绝
          (checkAccess)：当前用户无权读取报告资源，请联系管理员。
        </div>
      )}
            ...
            <button
              onClick={() => {
                if (selectedIds.size > 0) {
                  const sel = allReports.find(
                    (r) => selectedIds.has(r.id) && r.status === "待审核",
                  );
          ...
          <button
            ...
            onClick={async (evt) => {
              ...
              const reports = JSON.parse(
                localStorage.getItem("g005_reports") || "[]",
              );
              reports.push({
                id: `R${Date.now()}`,
      ...
      <div
        className="no-print"
        style={{ maxWidth: 1440, margin: "0 auto", padding: "20px 24px" }}
      >
        {/* 统计卡片行 */}
        <div
        ...
        >
          ...
          <div
            style={{ display: "flex", gap: 4, flexWrap: "wrap", maxWidth: 600 }}
          >
        ...
        <div className="report-filters">
          <FilterBar
            ...
            reportDoctorFilter={reportDoctorFilter}
            setReportDoctorFilter={setReportDoctorFilter}
        ...
        <div
          ...
          style={{
            marginBottom: showAdvancedFilter ? 14 : 0,
            transition: "all 0.2s",
          }}
        >
          <button
            ...
            style={{
              padding: "6px 14px",
              borderRadius: 8,
              border: `1px solid ${showAdvancedFilter ? "#1e40af" : "#e2e8f0"}`,
              background: showAdvancedFilter ? "#eff6ff" : WHITE,
              color: showAdvancedFilter ? "#1e40af" : GRAY,
              fontSize: 12,
              fontWeight: 600,
              cursor: "pointer",
              display: "flex",
              alignItems: "center",
              gap: 5,
              marginBottom: showAdvancedFilter ? 10 : 0,
            }}
          >
          ...
          {showAdvancedFilter && (
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                ...
                <input
                  type="number"
                  value={qualityScoreFrom}
                  onChange={(e) =>
                    setQualityScoreFrom(Number(e.target.value) || 0)
                  }
                  min={0}
                  max={100}
                  style={{
                ...
                <input
                  ...
                  onChange={(e) =>
                    setQualityScoreTo(Number(e.target.value) || 100)
                  }
                  min={0}
                  max={100}
                  style={{
        ...
        >
          ...
          <div
            style={{ display: "flex", gap: 6, marginLeft: 8, flexWrap: "wrap" }}
          >
            {[
              {
                label: "全部",
              ...
              {
                label: "待分配",
                count: filteredReports.filter((r) => r.status === "待分配")
                  .length,
                color: "#6b7280",
                key: "待分配",
              },
              {
                label: "已分配",
                count: filteredReports.filter((r) => r.status === "已分配")
                  .length,
                color: "#0369a1",
                key: "已分配",
              },
              {
                label: "书写中",
                count: filteredReports.filter((r) => r.status === "书写中")
                  .length,
                color: "#1e40af",
                key: "书写中",
              },
              {
                label: "初审中",
                count: filteredReports.filter((r) => r.status === "初审中")
                  .length,
                color: "#7c2d12",
                key: "初审中",
              },
              {
                label: "终审中",
                count: filteredReports.filter((r) => r.status === "终审中")
                  .length,
                color: "#a16207",
                key: "终审中",
              },
              {
                label: "已签发",
                count: filteredReports.filter((r) => r.status === "已签发")
                  .length,
                color: "#047857",
                key: "已签发",
              },
          ...
          >
            {selectedIds.size > 0 && (
              <>
                ...
                <button
                  onClick={() => {
                    const toPrint = allReports.filter((r) =>
      ...
      {reviewResultModal.show && (
        <div
          ...
          onClick={() => setReviewResultModal((r) => ({ ...r, show: false }))}
        >
          <div
            style={{
              background: WHITE,
          ...
          >
            ...
            >
              ...
              <button
                onClick={() =>
                  setReviewResultModal((r) => ({ ...r, show: false }))
                }
                style={{
                  background: "none",
                  border: "none",
            ...
            >
              <CheckCircle
                size={32}
                color={reviewResultModal.result === "已审核" ? SUCCESS : DANGER}
              />
              <div>
                <div
                  style={{
                    ...
                    fontWeight: 700,
                    color:
                      reviewResultModal.result === "已审核" ? SUCCESS : DANGER,
                  }}
                >
                  {reviewResultModal.result === "已审核"
                    ? "审核通过"
                    : "已退回"}
                </div>
                <div style={{ fontSize: 12, color: GRAY }}>
                  报告ID: {reviewResultModal.reportId}
                </div>
              </div>
            </div>
            <div
      ...
      {batchResultModal.show && (
        <div
          ...
          onClick={() => setBatchResultModal((b) => ({ ...b, show: false }))}
        >
          <div
            style={{
              background: WHITE,
          ...
          >
            ...
            >
              ...
              >
                {batchResultModal.title}
              </h2>
              <button
                onClick={() =>
                  setBatchResultModal((b) => ({ ...b, show: false }))
                }
                style={{
                  background: "none",
                  border: "none",
      ...
      {printModal.show && (
        <div
          ...
          onClick={() => setPrintModal((p) => ({ ...p, show: false }))}
        >
          <div
            style={{
              background: WHITE,
      ...
      {bulkActionModal.show && (
        <div
          ...
          onClick={() => setBulkActionModal((b) => ({ ...b, show: false }))}
        >
          <div
            style={{
              background: WHITE,
          ...
          >
              ...
              <div>
                <h2
                  style={{
                    ...
                    fontWeight: 800,
                    color:
                      bulkActionModal.action === "delete" ? DANGER : PRIMARY,
                  }}
                >
                ...
                <div style={{ fontSize: 12, color: GRAY, marginTop: 2 }}>
                  将{bulkActionModal.action === "delete" ? "删除" : "发布"}{" "}
                  {bulkActionModal.count} 份报告
                </div>
              </div>
            </div>
            {bulkActionModal.action === "delete" && (
            ...
            >
              <button
                onClick={() =>
                  setBulkActionModal((b) => ({ ...b, show: false }))
                }
                style={{
                  padding: "8px 20px",
                  border: "1px solid #e2e8f0",
                ...
                onClick={() => {
                  ...
                  setTimeout(() => {
                    if (action === "publish") {
                      setAllReports((prev) =>
                        prev.map((r) =>
                          selectedIds.has(r.id) && r.status === "待审核"
                            ? {
                                ...r,
                                status: "已发布",
                    ...
                    setSelectedIds(new Set());
                    setBulkActionModal((b) => ({
                      ...b,
                      show: false,
                      loading: false,
                    }));
                    showToast(
                      `${action === "publish" ? "发布" : "删除"}成功`,
                      "success",
                    );
                  }, 1000);
                }}
              ...
              >
                {bulkActionModal.loading ? (
                  ...
                  <>
                    确认{bulkActionModal.action === "delete" ? "删除" : "发布"}
                  </>
                )}
              </button>
            </div>

...
import {
  ChevronLeft, ChevronRight, CheckSquare, Square, Search, Eye, Edit2, PlusCircle, FileText,
  Download, Printer, X, GitFork, User, Phone, CreditCard, Calendar, MapPin, Contact,
  Shield, Activity, AlertTriangle, AlertCircle, CheckCircle
} from 'lucide-react'
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <button onClick={() => onPageChange(currentPage - 1)} disabled={currentPage === 1}
          style={{ width: 32, height: 32, borderRadius: 6, border: '1px solid #e2e8f0', background: '#fff', cursor: currentPage === 1 ? 'not-allowed' : 'pointer', opacity: currentPage === 1 ? 0.5 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
          ...
          return (
            <button key={pageNum} onClick={() => onPageChange(pageNum)}
              style={{ minWidth: 32, height: 32, borderRadius: 6, border: '1px solid', borderColor: currentPage === pageNum ? '#1e3a5f' : '#e2e8f0', background: currentPage === pageNum ? '#1e3a5f' : '#fff', color: currentPage === pageNum ? '#fff' : '#64748b', cursor: 'pointer', fontSize: 12, fontWeight: 600, padding: '0 8px' }}>
              {pageNum}
            </button>
          )
        })}
        <button onClick={() => onPageChange(currentPage + 1)} disabled={currentPage === totalPages}
          style={{ width: 32, height: 32, borderRadius: 6, border: '1px solid #e2e8f0', background: '#fff', cursor: currentPage === totalPages ? 'not-allowed' : 'pointer', opacity: currentPage === totalPages ? 0.5 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
...
export function PatientTable({
  patients, paginatedPatients, filteredPatientsLength,
  selectedPatientIds, onSelectionChange,
  currentPage, totalPages, pageSize, onPageChange,
  onViewPatient, onEditPatient, exams,
  visibleDuplicates, onDismissAllDuplicates,
  selectedPatient, onSelectPatient, onToast,
}: PatientTableProps) {
  ...
  const handleBulkExport = () => {
    ...
    onToast({ show: true, type: 'success', message: `成功导出 ${selected.length} 条患者记录` })
  }

  const handleBulkPrint = () => {
    onToast({ show: true, type: 'info', message: `已发送 ${selectedPatientIds.size} 个标签到打印队列` })
    onSelectionChange(new Set())
  }
  ...
  return (
    <>
      {visibleDuplicates.length > 0 && (
        <div style={{
          marginBottom: 16, padding: '12px 16px', background: '#fffbeb',
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button onClick={onDismissAllDuplicates} style={{ padding: '4px 10px', borderRadius: 6, border: '1px solid #e2e8f0', background: '#fff', fontSize: 11, cursor: 'pointer', color: '#64748b' }}>
              <X size={12} /> 忽略
            </button>
          </div>
        </div>
      ...
      {selectedPatientIds.size > 0 && (
        <div style={{
          ...
          <button onClick={() => onSelectionChange(new Set())} style={{ marginLeft: 'auto', padding: '6px 12px', borderRadius: 6, background: 'transparent', border: '1px solid rgba(255,255,255,0.2)', fontSize: 12, color: 'rgba(255,255,255,0.7)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
            <X size={12} />清除
          </button>
        </div>
      )}
      ...
      <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e2e8f0', overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12, minWidth: 1150 }}>
            <thead>
              <tr style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
                <th scope="col" style={{ padding: '12px 10px', width: 40, textAlign: 'center' }}>
                  <div onClick={toggleSelectAll} role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); toggleSelectAll(); } }} style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: allSelected ? '#1e3a5f' : '#cbd5e1' }}>
            ...
            <tbody>
              {paginatedPatients.map((p, idx) => {
                ...
                return (
                  <tr key={p.id}
                    style={{ borderBottom: '1px solid #f1f5f9', cursor: 'pointer', background: isSelected ? '#f0f7ff' : idx % 2 === 0 ? '#fff' : '#fafbfc' }}
                  ...
                  >
                    ...
                    <td style={{ padding: '10px 14px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <div style={{ width: 28, height: 28, borderRadius: '50%', background: p.gender === '男' ? '#dbeafe' : '#fce7f3', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, color: p.gender === '男' ? '#1e3a5f' : '#be185d' }}>
                          {p.name.slice(0, 1)}
                        </div>
                    ...
                    <td style={{ padding: '10px 14px' }}>
                      <span style={{ padding: '2px 8px', borderRadius: 4, fontSize: 11, fontWeight: 600, background: p.gender === '男' ? '#dbeafe' : '#fce7f3', color: p.gender === '男' ? '#1e40af' : '#be185d' }}>
                        {p.gender}
                      </span>
                    </td>
                    ...
                    <td style={{ padding: '10px 14px', textAlign: 'center', fontWeight: 700, color: '#1e3a5f' }}>{p.totalExamCount || 0}</td>
                    <td style={{ padding: '10px 14px', color: '#64748b', fontSize: 11 }}>{p.lastExamDate || '-'}</td>
                    <td style={{ padding: '10px 14px' }}>
                      <div style={{ display: 'flex', gap: 4 }}>
                        <button onClick={(e) => { e.stopPropagation(); onViewPatient(p) }}
                          style={{ padding: '4px 8px', background: '#eff6ff', color: '#2563eb', border: 'none', borderRadius: 4, fontSize: 11, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 3 }}>
                          <Eye size={12} />查看
                        </button>
                        <button onClick={(e) => { e.stopPropagation(); onEditPatient(p) }}
                          style={{ padding: '4px 8px', background: '#f0fdf4', color: '#16a34a', border: 'none', borderRadius: 4, fontSize: 11, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 3 }}>
                          <Edit2 size={12} />编辑
                        </button>
                        <button onClick={(e) => { e.stopPropagation() }}
                          style={{ padding: '4px 8px', background: '#fef3c7', color: '#d97706', border: 'none', borderRadius: 4, fontSize: 11, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 3 }}>
                          <PlusCircle size={12} />检查
                        </button>
                        <button onClick={(e) => { e.stopPropagation() }}
                          style={{ padding: '4px 8px', background: '#f5f3ff', color: '#7c3aed', border: 'none', borderRadius: 4, fontSize: 11, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 3 }}>
                          <FileText size={12} />报告
                        </button>
                      </div>
                    </td>
        ...
        <Pagination currentPage={currentPage} totalPages={totalPages} totalItems={filteredPatientsLength} pageSize={pageSize} onPageChange={onPageChange} />
      </div>

      {selectedPatient && (
        <div style={{ marginTop: 16, background: '#fff', borderRadius: 12, border: '1px solid #e2e8f0', padding: 20, boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ width: 56, height: 56, borderRadius: '50%', background: 'linear-gradient(135deg, #1e3a5f, #3b82f6)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 12, color: '#64748b' }}>{selectedPatient.gender} · {selectedPatient.age}岁 · {selectedPatient.patientType}</div>
              </div>
            </div>
            <button onClick={() => onSelectPatient(null)} style={{ width: 32, height: 32, borderRadius: 8, border: '1px solid #e2e8f0', background: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          {selectedPatient.allergyHistory && selectedPatient.allergyHistory !== '无' && (
            <div style={{ marginTop: 16, padding: '12px 16px', background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 8, display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <div style={{ marginTop: 16 }}>
            ...
            <div style={{ fontSize: 12, color: '#334155', padding: 12, background: '#f8fafc', borderRadius: 8 }}>{selectedPatient.medicalHistory || '无'}</div>
          </div>
        </div>
      )}
    </>

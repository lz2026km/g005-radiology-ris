...
import { Save, ArrowLeft, UserPlus, ChevronLeft, ChevronRight, CheckCircle, X } from 'lucide-react'
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ padding: '20px 24px', borderBottom: '1px solid #e2e8f0', background: 'linear-gradient(135deg, #1e3a5f, #3b82f6)' }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
            {[1, 2, 3].map(s => (
              <div key={s} style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 8 }}>
                <div style={{ width: 24, height: 24, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, background: step >= s ? '#fff' : 'rgba(255,255,255,0.3)', color: step >= s ? '#1e3a5f' : 'rgba(255,255,255,0.6)' }}>{s}</div>
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  {(['男', '女'] as GenderFilter[]).map(g => (
                    <button key={g} onClick={() => setFormData({ ...formData, gender: g })} style={{ flex: 1, padding: '10px 0', borderRadius: 8, border: `1px solid ${formData.gender === g ? '#1e3a5f' : '#e2e8f0'}`, background: formData.gender === g ? '#1e3a5f' : '#fff', color: formData.gender === g ? '#fff' : '#64748b', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>{g}</button>
                  ))}
                </div>
              </div>
              <div>
...
export function PatientCreateForm({ selectedPatientForEdit, formData, formErrors, onFormDataChange, onSave, onCancel }: PatientCreateFormProps) {
  return (
    <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e2e8f0', padding: 24, boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
        <button onClick={onCancel} style={{ width: 36, height: 36, borderRadius: 8, border: '1px solid #e2e8f0', background: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <ArrowLeft size={16} color="#64748b" />
        </button>
        <div>
          <div style={{ fontSize: 16, fontWeight: 700, color: '#1e3a5f' }}>{selectedPatientForEdit ? '编辑患者信息' : '新建患者档案'}</div>
          <div style={{ fontSize: 12, color: '#64748b', marginTop: 2 }}>{selectedPatientForEdit ? `患者ID: ${selectedPatientForEdit.id}` : '请填写以下信息'}</div>
        </div>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
        <div>
          <label style={{ fontSize: 12, fontWeight: 600, color: '#334155', marginBottom: 6, display: 'block' }}>患者姓名 <span style={{ color: '#dc2626' }}>*</span></label>
          <input type="text" value={formData.name} onChange={e => onFormDataChange({ ...formData, name: e.target.value })} placeholder="请输入患者姓名"
            style={{ width: '100%', padding: '10px 12px', borderRadius: 8, border: `1px solid ${formErrors.name ? '#dc2626' : '#e2e8f0'}`, fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />
          {formErrors.name && <div style={{ fontSize: 11, color: '#dc2626', marginTop: 4 }}>{formErrors.name}</div>}
        </div>
        <div>
          <label style={{ fontSize: 12, fontWeight: 600, color: '#334155', marginBottom: 6, display: 'block' }}>性别 <span style={{ color: '#dc2626' }}>*</span></label>
          <div style={{ display: 'flex', gap: 8 }}>
            {(['男', '女'] as GenderFilter[]).map(g => (
              <button key={g} onClick={() => onFormDataChange({ ...formData, gender: g })}
                style={{ flex: 1, padding: '10px 0', borderRadius: 8, border: `1px solid ${formData.gender === g ? '#1e3a5f' : '#e2e8f0'}`, background: formData.gender === g ? '#1e3a5f' : '#fff', color: formData.gender === g ? '#fff' : '#64748b', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>{g}</button>
            ))}
          </div>
        </div>
        <div>
          <label style={{ fontSize: 12, fontWeight: 600, color: '#334155', marginBottom: 6, display: 'block' }}>年龄</label>
          <input type="number" value={formData.age} onChange={e => onFormDataChange({ ...formData, age: e.target.value })} placeholder="请输入年龄"
            style={{ width: '100%', padding: '10px 12px', borderRadius: 8, border: '1px solid #e2e8f0', fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />
        </div>
        <div>
          <label style={{ fontSize: 12, fontWeight: 600, color: '#334155', marginBottom: 6, display: 'block' }}>身份证号 <span style={{ color: '#dc2626' }}>*</span></label>
          <input type="text" value={formData.idCard} onChange={e => onFormDataChange({ ...formData, idCard: e.target.value })} placeholder="请输入18位身份证号" maxLength={18}
            style={{ width: '100%', padding: '10px 12px', borderRadius: 8, border: `1px solid ${formErrors.idCard ? '#dc2626' : '#e2e8f0'}`, fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />
          {formErrors.idCard && <div style={{ fontSize: 11, color: '#dc2626', marginTop: 4 }}>{formErrors.idCard}</div>}
        </div>
        <div>
          <label style={{ fontSize: 12, fontWeight: 600, color: '#334155', marginBottom: 6, display: 'block' }}>联系电话 <span style={{ color: '#dc2626' }}>*</span></label>
          <input type="tel" value={formData.phone} onChange={e => onFormDataChange({ ...formData, phone: e.target.value })} placeholder="请输入手机号" maxLength={11}
            style={{ width: '100%', padding: '10px 12px', borderRadius: 8, border: `1px solid ${formErrors.phone ? '#dc2626' : '#e2e8f0'}`, fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />
          {formErrors.phone && <div style={{ fontSize: 11, color: '#dc2626', marginTop: 4 }}>{formErrors.phone}</div>}
        </div>
        <div>
          <label style={{ fontSize: 12, fontWeight: 600, color: '#334155', marginBottom: 6, display: 'block' }}>患者类型</label>
          <select value={formData.patientType} onChange={e => onFormDataChange({ ...formData, patientType: e.target.value as PatientTypeFilter })}
        ...
        <div style={{ gridColumn: '1 / -1' }}>
          <label style={{ fontSize: 12, fontWeight: 600, color: '#334155', marginBottom: 6, display: 'block' }}>家庭住址</label>
          <input type="text" value={formData.address} onChange={e => onFormDataChange({ ...formData, address: e.target.value })} placeholder="请输入详细地址"
            style={{ width: '100%', padding: '10px 12px', borderRadius: 8, border: '1px solid #e2e8f0', fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />
        </div>
        <div>
          <label style={{ fontSize: 12, fontWeight: 600, color: '#334155', marginBottom: 6, display: 'block' }}>联系人姓名 <span style={{ color: '#dc2626' }}>*</span></label>
          <input type="text" value={formData.emergencyContact} onChange={e => onFormDataChange({ ...formData, emergencyContact: e.target.value })} placeholder="请输入联系人姓名"
            style={{ width: '100%', padding: '10px 12px', borderRadius: 8, border: `1px solid ${formErrors.emergencyContact ? '#dc2626' : '#e2e8f0'}`, fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />
          {formErrors.emergencyContact && <div style={{ fontSize: 11, color: '#dc2626', marginTop: 4 }}>{formErrors.emergencyContact}</div>}
        </div>
        <div>
          <label style={{ fontSize: 12, fontWeight: 600, color: '#334155', marginBottom: 6, display: 'block' }}>联系人电话 <span style={{ color: '#dc2626' }}>*</span></label>
          <input type="tel" value={formData.emergencyPhone} onChange={e => onFormDataChange({ ...formData, emergencyPhone: e.target.value })} placeholder="请输入联系人电话" maxLength={11}
            style={{ width: '100%', padding: '10px 12px', borderRadius: 8, border: `1px solid ${formErrors.emergencyPhone ? '#dc2626' : '#e2e8f0'}`, fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />
          {formErrors.emergencyPhone && <div style={{ fontSize: 11, color: '#dc2626', marginTop: 4 }}>{formErrors.emergencyPhone}</div>}
        </div>
        <div>
          <label style={{ fontSize: 12, fontWeight: 600, color: '#334155', marginBottom: 6, display: 'block' }}>医保类型</label>
          <select value={formData.insuranceType} onChange={e => onFormDataChange({ ...formData, insuranceType: e.target.value })}
            style={{ width: '100%', padding: '10px 12px', borderRadius: 8, border: '1px solid #e2e8f0', fontSize: 13, outline: 'none', background: '#fff', boxSizing: 'border-box' }}>
            <option value="">请选择</option>
            <option value="城镇职工基本医疗保险">城镇职工基本医疗保险</option>
            <option value="城乡居民基本医疗保险">城乡居民基本医疗保险</option>
        ...
        <div>
          <label style={{ fontSize: 12, fontWeight: 600, color: '#334155', marginBottom: 6, display: 'block' }}>床位号</label>
          <input type="text" value={formData.bedNumber} onChange={e => onFormDataChange({ ...formData, bedNumber: e.target.value })} placeholder="如：3床"
            style={{ width: '100%', padding: '10px 12px', borderRadius: 8, border: '1px solid #e2e8f0', fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />
        </div>
        <div style={{ gridColumn: '1 / -1' }}>
          <label style={{ fontSize: 12, fontWeight: 600, color: '#334155', marginBottom: 6, display: 'block' }}>主治医师</label>
          <input type="text" value={formData.attendingDoctor} onChange={e => onFormDataChange({ ...formData, attendingDoctor: e.target.value })} placeholder="请输入主治医师姓名"
            style={{ width: '100%', padding: '10px 12px', borderRadius: 8, border: '1px solid #e2e8f0', fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />
        </div>
        <div>
          <label style={{ fontSize: 12, fontWeight: 600, color: '#334155', marginBottom: 6, display: 'block' }}>过敏史</label>
          <input type="text" value={formData.allergyHistory} onChange={e => onFormDataChange({ ...formData, allergyHistory: e.target.value })} placeholder="请输入过敏史（无则填'无'）"
            style={{ width: '100%', padding: '10px 12px', borderRadius: 8, border: '1px solid #e2e8f0', fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />
        </div>
        <div>
          <label style={{ fontSize: 12, fontWeight: 600, color: '#334155', marginBottom: 6, display: 'block' }}>既往史</label>
          <input type="text" value={formData.medicalHistory} onChange={e => onFormDataChange({ ...formData, medicalHistory: e.target.value })} placeholder="请输入既往病史"
            style={{ width: '100%', padding: '10px 12px', borderRadius: 8, border: '1px solid #e2e8f0', fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />
        </div>
      </div>

      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12, marginTop: 32, paddingTop: 24, borderTop: '1px solid #e2e8f0' }}>
        <button onClick={onCancel} style={{ padding: '12px 24px', borderRadius: 8, border: '1px solid #e2e8f0', background: '#fff', color: '#64748b', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>取消</button>
        <button onClick={onSave} style={{ padding: '12px 24px', borderRadius: 8, border: 'none', background: '#1e3a5f', color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8 }}>
          <Save size={16} />保存患者信息
        </button>
      </div>
    </div>

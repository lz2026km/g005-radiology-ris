// v3.0.6.8-44 Comprehensive E2E Test (ASCII only)
...
if (r8b.ok) ok('TURN/5G', r8b.body.data['5G_edge']?.slice || '5G');
else failT('TURN', '');

const r8c = await callApi(p, API + '/eye/optometry/refraction', { method: 'POST', body: { patientId: 'P001', reSphere: -2.5, reCylinder: -0.75, reAxis: 180, leSphere: -2.75, leCylinder: -1, leAxis: 175, prescriptionType: 'glasses' } });
...
const r9b = await callApi(p, API + '/eye/edu/annotate', { method: 'POST', body: { caseId: 'CASE001', annotationType: 'roi', coordinates: [[100,100]], label: 'disc' } });
if (r9b.ok && r9b.body?.data?.annotationId) ok('Annotate', r9b.body.data.annotationId);
...
if (r11d.ok) ok('Ortho-K order', 'CNY ' + r11d.body.data.cost.total);
else failT('Ortho-K', '');

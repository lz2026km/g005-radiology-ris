// smart-icon-fix-v6.mjs
// V6: Strip comments from import blocks before parsing
...
// Strip comments (// to end of line, and /* ... */) from a string
...
function parseLucideImports(importBlock) {
  // importBlock is the content inside {...} from a lucide-react import
  // Returns array of { original, local, raw }
...
function fixFile(info) {
  ...
  // Match the import WITHOUT consuming trailing newlines.
  // Group 1: content inside {}; Group 2: optional trailing ';' (we restore it).
  ...
  // Build new parts preserving the original "raw" form (alias included)
  const newParts = existingParts.map(p => p.raw);
  // Add missing icons (alphabetical, only those not already present)
  ...
  // Detect whether original had a semicolon (we'll preserve it)

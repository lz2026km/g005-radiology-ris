...
function isLocalDefinition(content, name) {
  const patterns = [
    ...
    // any prop in destructuring: { ..., X, ... } or { ..., X: ... } or { ..., prop: X, ... }
...
function analyzeFile(filepath) {
  ...
  // Find JSX usages: <Xxx ...> or <Xxx/> where Xxx is followed by space, /, >, or end
  // Exclude generic type annotations like <Xxx[]> or <Xxx<X>>
  // Match < then PascalCase identifier, then NOT [ (which would be generic)

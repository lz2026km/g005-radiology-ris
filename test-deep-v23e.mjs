...
const API_NOISE = ['/api/v1/', 'Failed to load resource', 'stats/daily']; // v27: MSW mock 不在 preview 模式, 忽略 API 500
const isNoise = (m) => CSP_NOISE.some(k => m.includes(k)) || API_NOISE.some(k => m.includes(k));
...
async function audit(page, path, useClick) {
  ...
  const onC = (m) => {
    ...
    if (m.type() === 'error') r.consoleErrors.push(t);
      ...
      const onR = (resp) => { if (resp.status() >= 400 && !resp.url().includes('favicon') && !resp.url().includes('mockServiceWorker') && !resp.url().includes('/api/v1/')) r.networkFails.push(`${resp.status()} ${resp.url().slice(-80)}`); };

...
const PAGES = [
  ...
  { n: '角膜地形', p: '/eye/pacs/topography' }, { n: 'FFA', p: '/eye/pacs/ffa' }, { n: '对比', p: '/eye/pacs/compare' },
  { n: '拼图', p: '/eye/pacs/montage' }, { n: '查看器', p: '/eye/pacs/viewer?studyId=es-001' },
  { n: 'RIS流程', p: '/eye/ris' }, { n: '报告书写', p: '/eye/report-write' },
  { n: 'IOL', p: '/eye/ris/iol-calculator' }, { n: '视力', p: '/eye/ris/va' },
  { n: '眼压', p: '/eye/ris/iop' }, { n: 'EMR', p: '/eye/emr' }, { n: 'AI', p: '/eye/ai' },
  { n: '质控看板', p: '/eye/kpi-dashboard' },
];
let ok = 0, fail = 0;
for (const pg of PAGES) {
  try {
    ...
    console.log(`  ${status} ${pg.n}: ${bodyText.length}ch`);
  } catch (e) { console.log(`  ✗ ${pg.n}: ${e.message.substring(0, 60)}`); fail++; }
}
